...
async function GET(request) {
    ...
    // Filter by keyword
    if (keyword) {
        todos = todos.filter((todo)=>String(todo.todo).toLowerCase().includes(keyword.toLowerCase()));
    }
    // Filter by status
    if (status) {
        todos = todos.filter((todo)=>String(todo.status) === status);
    }
    // Sort by field
    if (sortField === "todo") {
        todos.sort((a, b)=>order === "asc" ? a.todo.localeCompare(b.todo) // if asc => a vs b else desc => b vs a
...
//# sourceMappingURL=%5Broot-of-the-server%5D__10a7768e._.js.map